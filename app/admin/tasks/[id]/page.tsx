...
import { ArrowLeft, Plus, Trash2, Link2, FileText, Users, CheckSquare, Square, Calendar, MessageSquare, Send, X, CheckCircle } from 'lucide-react'
...
export default function AdminTaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [resourceForm, setResourceForm] = useState({ type: 'URL', title: '', url: '' })
  ...
  async function addSubtask(e: React.FormEvent) {
    e.preventDefault(); if (!subtaskInput.trim()) return
    setSaving('subtask')
    await fetch(`/api/tasks/${id}/subtasks`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: subtaskInput.trim() }) })
    setSubtaskInput(''); setSaving(null); load()
  }
  ...
  async function addResource(e: React.FormEvent) {
    ...
    setResourceForm({ type: 'URL', title: '', url: '' }); setSaving(null); load()
  }
  ...
  const assigneeIds = new Set(task.assignments.map((a) => a.user.id))
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex flex-col gap-1 text-right">
            ...
            {task.deadline && <span className="text-xs text-charcoal/50 flex items-center gap-1 justify-end"><Calendar size={11} /> {new Date(task.deadline).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="flex flex-col gap-2 mb-3">
              {task.subtasks.map((s) => (
                <div key={s.id} className="flex items-center gap-3 group">
                  ...
                  <span className="text-sm text-midnight flex-1">{s.title}</span>
                  <button onClick={() => deleteSubtask(s.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-50 text-charcoal/30 hover:text-red-500 transition-all">
                    <X size={13} />
                  </button>
                </div>
              ))}
            ...
            <form onSubmit={addSubtask} className="flex gap-2">
              ...
              <Button type="submit" size="sm" loading={saving === 'subtask'} className="flex items-center gap-1"><Plus size={13} /> Add</Button>
            </form>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="flex flex-col gap-2 mb-4">
              {task.resources.map((r) => (
                <div key={r.id} className="flex items-center gap-3 p-3 bg-midnight/5 rounded-xl group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <a href={r.url} target="_blank" rel="noreferrer" className="text-xs text-charcoal/50 hover:text-midnight truncate block transition-colors">{r.url.slice(0, 60)}{r.url.length > 60 ? '…' : ''}</a>
                  </div>
                  <button onClick={() => deleteResource(r.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-50 text-charcoal/30 hover:text-red-500 transition-all">
                    <Trash2 size={13} />
                  </button>
                </div>
              ))}
            ...
            <form onSubmit={addResource} className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <input value={resourceForm.url} onChange={(e) => setResourceForm((f) => ({ ...f, url: e.target.value }))} placeholder="https://…"
                  className="flex-1 px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-midnight" />
                <Button type="submit" size="sm" loading={saving === 'resource'} className="flex items-center gap-1"><Plus size={13} /> Add</Button>
              </div>
            </form>
          </div>
        ...
        {/* Right col: assign educators + progress */}
