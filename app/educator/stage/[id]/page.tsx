...
interface Stage {
  ...
  docUrl: string | null; timeLimitMinutes: number; passScore: number
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {phase === 'doc' && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <h2 className="font-bold text-midnight text-lg mb-3">Step 1: Read the Training Document</h2>
          <p className="text-sm text-charcoal/60 mb-5">
            Read the training document carefully before taking the MCQ. Take your time — understanding the material will help you pass.
          </p>

          {stage.docUrl ? (
            <>
              <a href={stage.docUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 text-sm font-semibold text-midnight bg-cream hover:bg-gold/20 border border-gold/30 px-5 py-3 rounded-xl transition-colors mb-5">
                <ExternalLink size={16} className="text-gold" />
                Open Training Document
              </a>
              <Button onClick={markDocRead} className="w-full">
                <CheckCircle size={16} /> I&apos;ve Read the Document — Start MCQ
              </Button>
            </>
          ) : (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700">
              Training document not yet uploaded. Contact your admin.
            </div>
          )}
        </div>
      )}
